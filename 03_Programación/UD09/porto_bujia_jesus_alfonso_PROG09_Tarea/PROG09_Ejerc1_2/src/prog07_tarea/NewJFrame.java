...
De acuerdo con la tarea de la unidad 9, ejercicio 1: Se incluye en el constructor la inicialización del objeto banco de acuerdo al contenido guardado en el fichero 
datoscuentasbancarias.dat. Se incluye también en el constructor un bucle para cargar en el combobox las cuentas ya existentes en el fichero y el objeto serialización 
que llega desde la clase principal como parámetro. En el botón de salir se introduce el método escribirObjeto(banco), por el cual se vuelca el contenido de la 
estructura de datos con las cuentas bancarias, por medio del objeto banco, al fichero datoscuentasbancarias.dat.  
Para el ejercicio 2 creo un nuevo botón llamado Listado Cliente, que al pulsarlo genera un fichero de texto denominado ListadoClientesCCC.txt 
que contiene una línea de texto por cada cuenta bancaria almacenada,y en la última línea del fichero se incluye el número total de cuentas existentes.
 */
public class NewJFrame extends javax.swing.JFrame {

    //Constructor con un parámetro objeto banco y serializadora, donde se define como se muestra el menú inicial
    public NewJFrame(Banco b, Serializadora serializadora) {
        initComponents();
        /*Condicional por el cual, si el fichero datoscuentasbancarias.dat no existe inicialmente, se crea un objeto banco vacío. 
        Una vez creado el fichero .dat, el banco tomará el valor de este fichero que entra como parámetro desde la 
        clase principal*/
        if (b != null) {
            banco = b;
        } else {
        ...
        s = serializadora;
        //Bucle for para añadir al combobox las cuentas ya creadas que están en el archivo "datoscuentasbancarias.dat"
        for (CuentaBancaria c : banco.cuentasTotales) {
            ListadoCuentas.addItem(c.getIBAN());
        }
    ...
    /*Método para salir de la aplicación, que se acciona pulsando un botón.
      Se incluye método escribirObjeto(banco), por el cual se vuelca el contenido de la estructura de datos 
      con las cuentas bancarias, por medio del objeto banco, al fichero datoscuentasbancarias.dat.    
     */
    private void SalirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SalirActionPerformed
        s.escribirObjeto(banco);
        System.exit(0);
    ...
    /*Método que genera un fichero de texto denominado ListadoClientesCCC.txt que contiene una línea de texto por cada cuenta bancaria almacenada, 
    donde se visualiza el nombre del propietario y CCC por cada una de ellas. En la última línea del fichero se incluye el número total de cuentas existentes.
     */
    private void listadoClientesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_listadoClientesActionPerformed
        int contador = 0;
        try {
            FileWriter fw = new FileWriter("ListadoClientesCCC.txt");
            for (CuentaBancaria c : banco.cuentasTotales) {
                fw.write("Nombre: " + c.getPersona().getNombre() + ", apellidos: " + c.getPersona().getApellidos() + ", CCC: " + c.getIBAN() + "\n");
                contador++;
            }
            fw.write("El número total de cuentas existentes ese: " + contador);
            fw.close();
            JOptionPane.showMessageDialog(this, "Fichero de texto ListadoClientesCCC.txt generado", "Información", JOptionPane.INFORMATION_MESSAGE);
